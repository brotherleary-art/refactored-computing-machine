...
describe('scouting route planner', () => {
  it('respects time, danger, discovered-site limits, and reward-density priority', () => {
    ...
    expect(route.stops.map(s => s.id)).toEqual(['camp']);
    expect(route.totalMinutes).toBe(8);
    expect(route.danger).toBe(5);
    expect(route.rewardScore).toBe(40);
  });
});
